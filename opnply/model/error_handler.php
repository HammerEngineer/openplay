...
class ErrorHandler{
   ...
   //set this user error handler method to ErrorHandler::Handler method
   ...
   public function GetBacktrace($irrelevantFirstEntries){
       $s ='';
       $MAXSTRLN = 64;
       ...
       foreach ($trace_array as $arr) {
           ...
           if(!empty($arr['args'])){
               foreach ($arr['args'] as $v) {
                   ...
                   {
                       $v = (string)@$v;
                       $str = htmlspecialchars(substr($v, 0, $MAXSTRLN));
                       if(strlen($v)> $MAXSTRLN){
                           $str .='....';
                           $args[] = '"'.$str.'"';
                       }
                   }
